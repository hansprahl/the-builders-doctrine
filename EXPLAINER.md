...
1. **The chassis.** The portable primitives — regex linters that catch observer-bias patterns at pre-commit time, refusal lists that say what the products will never build, reflection gates that audit every AI response against the product's commandments, an approval queue that gates every irreversible action. Any builder can clone and adopt these. The chassis is publicly versioned (v1.0 shipped 2026-05-21, 11 days early from the 2026-06-01 target); the moat is not the code itself but the discipline of running it against your own work.
