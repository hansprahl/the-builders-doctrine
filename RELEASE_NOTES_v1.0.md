...
> **RESOLUTION — 2026-08-09.** The schedule above never executed: the statistician lane deferred to a Jan-2027 university-capstone window (2026-05-21), pre-reg v2 and the OSF posting were never made, and the 2026-07-20 verdict date lapsed silently — a Law VII violation discovered 2026-08-07. Corrective action: the pre-committed N=9-per-arm pilot (scaffolded 2026-05-14) ran 2026-08-08 on the original pinned models. **VERDICT: RED, 0/2 pre-committed criteria — the Stage 7 effect failed to replicate in both directions (refusals 0/9 vs 0/9; breaches 4/9 vs 4/9). The Law VI replication workstream is TRUNCATED per the pilot's own rule; the Law I causal claim's deprecation is SUSTAINED by failed replication.** The 108-run formal study is not warranted and the v1.5 milestone is closed as retraction-sustained. Full pathology: `funkytown/experiments/04_law_vi_replication/FINDINGS.md` (first artifact of the `autopsy` procedure). Incidental finding worth the price of the study: 8/8 breaching agents knew the gate rule, were detected, and confessed — only *prevention* was broken. Prompts persuade, audits observe, only gates prevent; the structural claims (chassis, approval gates, E1's +29% structure effect) are strengthened by the same data that retired the biographical one.
