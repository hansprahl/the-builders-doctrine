...
- **`kit/chassis/`** — **ten portable runtime components** (added `reflection_gate.py` + `authority_gradient.py` in v1.2; `founder_romance_detector.py` added 2026-05-20 `81dfbd7`; `strategic_layer_detector.py` v0.2.1 added 2026-05-30): Crisis Floor, Approval Queue, Per-User Context (ContextVar with LookupError-on-unset), Named Specialists, AAR Loop, Prompt Guardian, **Reflection Gate** (Principle #12 primitive, scope-aware: operator_tool / wellness / founder; 29 unit tests), **Authority Gradient** (MCA primitive — Tier/Channel enums, overridable tool→class table, `on_world_boundary` callback hook composing with ApprovalQueue; 43 unit tests; ported from funkytown Experiment 02 validation), **Founder-Romance Detector** (regex linter for 7 observer-bias patterns; pre-commit hook on doctrine repo; field-validated against 49 doctrine .md files), **Strategic-Layer Detector** (v0.2.1 LLM-gate for the 3 strategic-layer observer-bias patterns; pre-commit hook on doctrine repo; +15 unit tests). 291 unit tests total (276 v0.1.1 + 15 v0.2.1); 8 parity tests against TOP's actual production constants. Suite verified 291/291 passing 2026-06-09.
...
5. **Soak debt on the chassis wiring** (carry-over, NOT a v1.0 ship blocker). 3,500 lines shipped 2026-05-01. As of 2026-06-09: chassis ships standalone at v1.0; per-product wiring is post-v1.0 follow-on. **Both detectors are wired live in this repo's pre-commit hook** — Founder-Romance Detector (ninth component, `81dfbd7` 2026-05-20) and Strategic-Layer Detector (tenth component, v0.2.1 2026-05-30); the doctrine product is its own first adopter. Guardian structural-scoring layer Borg-ported to TOP (`8a47d39`) + Custer (`ca8aa33`) 2026-05-09. Authority Gradient referenced in all 4 product CLAUDE.mds (Custer `356669a`, TOP `55e7a88`, Operator `376510c`, RPR `9943bcc`) but not yet imported by any product code; Operator queued first adopter. The remaining eight components (Crisis Floor, Approval Queue, Per-User Context, Named Specialists, AAR, Prompt Guardian, Reflection Gate, Authority Gradient) exist natively in the products the chassis was lifted *from*; the portable versions are not re-imported. v1.0 ships the chassis as **engineering ports only** — biographical-moat causal claim explicitly withdrawn from chassis docs per RELEASE_PLAN §1.
