...
- `HANS_METHOD.md` — **HYPOTHESIS-GRADE, not ratified.** The epistemic ledger: Hold / Admit / Notice / Sweep over the four known-unknown cells, one non-substitutable mechanism each. Named 2026-08-10. Carries the measured portfolio record (~23 of 28 recorded failures in the two silently-failing cells; 64 kill headstones with instrument attribution; the AI inside the system found 1 of 64) and an honest prior-art section — **the four-cell matrix for AI is TAKEN** (CogniSwitch) and an earlier originality claim about "confident failure is the dangerous mode" is retracted in-file. Unclaimed and surviving: the sovereignty connection, non-substitutability, the operator-vs-system inversion, and the Sweep-requires-an-outside-mind tension resolved by gating what crosses. Four requirements owed before ratification; Law VII dates attach then, not now
- `COLLECTION_PIPELINE.md` — **KU, not Law.** Ingest-as-Law killed 2026-08-22 (H1a). Cut-to-`multi_search` killed (H1c). Epistemology KEEP. Do not fork `EPISTEMOLOGY.md`. Soak / CR-3 2026-08-24.
- `PROMPT_DOCTRINE.md` — universal prompt structural rules (canonical upstream)
- `BASELINE_CONSTANT.md` — **ratified 2026-08-12.** Scientific method. Untreated score before any treatment. Not a new Law. Parent is Law II.
