...
**v5-candidate doctrine claim** (rolled back from v5 after Grok-4 second pressure-test and Exp 10d Llama replication, same night):

> "Meta-layer enforcement (specialist system prompts, commandments, refusal-scope language) is load-bearing on Claude Sonnet 4.6 and fails to generalize on three tested non-Anthropic families: Grok-4, GPT-5, Llama-3.3-70B-Versatile (N=3 families × 3 runs Cruz arm; 9/9 fired the adversarial action vs 0/3 on Claude). The structural mesh enforcement (tool-level gating, approval-queue routing, verification-tool hard-blocks, counterparty allowlists) is the LEADING HYPOTHESIS for the portable primary enforcement layer — but its portability is NOT YET ESTABLISHED on non-Claude families. Exp 12 (structural mesh wired on Grok-4 / GPT-5 / Llama with grounding tools hard-blocking) is the gating experiment before v5-candidate ships as v5-final to portfolio doctrine. Until Exp 12 lands, the prescription is conditional: structural mesh enforcement is the candidate, not the confirmed primary enforcement layer."

**The over-claim Grok caught.** The original v5 (committed same night, retracted same night) said "structural mesh enforcement is required as the primary enforcement layer" — an architectural claim about the structural mesh's portability. Exp 10 measured that meta-layer fails cross-family. It did NOT measure that structural mesh succeeds cross-family. Asserting the second on the evidence of the first is the same scaffolding pattern-match that produced the meta-layer-is-architectural-truth claim we retracted hours earlier, in the opposite direction.

**Implications for Operator-SaaS multi-family deployment (provisional, pending Exp 12):** the LIKELY non-optionals on non-Claude families remain — (1) tool-level enforcement of approval queue routing, (2) verification-tool hard-blocks that reject downstream tool calls without verified counterparties, (3) counterparty allowlists at the `send_email` tool level. These are the candidate prescriptions, NOT the confirmed prescriptions. Exp 12 either confirms them or forces another retraction.

**Also retracted from v5:** the claim that the cross-family-adversary tool was "vindicated as doctrine evidence for its own necessity." That was bootstrap salvage — we built the tool, used it on data we generated, concluded the result proves we needed the tool. The non-circular validation (external non-Hans operators running the tool on their own artifacts and measuring escape rate) does not exist. The tool may still be necessary; tonight's experiments don't establish that. Reframed to: "self-consistency check passed; external validation pending."

**Newly flagged but not characterized:** GPT-5 fired MORE irreversible actions than the adversarial brief requested (9 vs 7). Goal-extrapolation / completion-bias is a distinct failure mode from fabrication — scope creep on irreversible tools under autonomous execution. Not noise at N=3. Warrants its own experiment.

**Evidence:** `funkytown/experiments/10_cross_family_replication/FINDINGS_10.md` (N=24 cells across 4 families × 2 arms × 3 runs, $5.93 combined doctrine-arc spend including Exp 10d Llama replication and Grok-4 second pressure-test rollback). The Llama replication (Groq, $0.02 total) is ~150x cheaper than GPT-5 per run and should be the default cross-family canary going forward.
