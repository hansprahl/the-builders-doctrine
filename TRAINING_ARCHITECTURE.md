...
- ~~That the T/C/S-as-eval framing has been ablation-tested the way MCA's Authority Gradient was in Funkytown. It has not.~~ **ABLATION-TESTED IN FUNKYTOWN, 2026-08-09/10 — and the result is graded, not binary.** Writing an explicit standard measurably changes agent outcomes: on a synthetic fixture, an explicit docstring band table moved defect detection from **0/12 to 12/12** (Fisher exact, p = 7.4×10⁻⁷); on **real third-party code**, a single prose sentence stating the expected value moved detection to **12/12**, against **6/12** for a sentence demanding the convention without supplying the value (p = 0.0137), **1/6** for no sentence at all, and **0/6** for a vague hint. **Stating the answer outperforms demanding the standard, which outperforms silence.** Two side effects nobody measured for: bug-certifying tests went 1→0, and manufactured findings went 5→0 — writing the spec down also suppressed fabrication. **NOT established:** dose-response below the sentence level; generalization across defect classes (one defect per corpus); or whether detections that transcribe the supplied text survive a strict no-transcription scorer — every winning assertion in the synthetic arm was a value printed verbatim in the treatment. Both experiments were narrowed by blind cross-family reads before this line was written; see `funkytown/experiments/23_spec_density_ablation/` and `.../24_real_code_replication/`.
